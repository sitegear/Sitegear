...
namespace Sitegear\Core\Module\Products;

use Sitegear\Base\Module\PurchaseItemProviderModuleInterface;
use Sitegear\Base\View\ViewInterface;
use Sitegear\Core\Module\AbstractCoreModule;
use Sitegear\Core\Module\Products\Model\Attribute;
use Sitegear\Core\Module\Products\Model\Item;
...
class ProductsModule extends AbstractCoreModule implements PurchaseItemProviderModuleInterface {
	...
	public function start() {
		...
		$this->getEngine()->doctrine()->getEntityManager()->getConfiguration()->addEntityNamespace(self::ENTITY_ALIAS, '\\Sitegear\\Core\\Module\\Products\\Model');
	}
	...
	public function getPurchaseItemAttributeDefinitions($type, $id) {
		...
		switch ($type) {
			case 'item':
				foreach ($this->getRepository('Item')->find($id)->getAttributeAssignments() as $assignment) { /** @var \Sitegear\Core\Module\Products\Model\AttributeAssignment $assignment */
					$values = array();
					foreach ($assignment->getAttribute()->getOptions() as $option) { /** @var \Sitegear\Core\Module\Products\Model\AttributeOption $option */
						$values[] = array(
							'id' => $option->getId(),
	...
	public function getPurchaseItemUnitPrice($type, $id, array $attributeValues) {
		...
		foreach ($attributeValues as $attributeValue) {
			/** @var \Sitegear\Core\Module\Products\Model\AttributeOption $attributeOption */
	 ...
	 * @param int $mode
	 * @param int $maxDepth
	 * @param null|int|\Sitegear\Core\Module\Products\Model\Category $parent
	 *
	 * @return array
	 */
